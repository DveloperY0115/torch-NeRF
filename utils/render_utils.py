...
def generate_rays(W: int, H: int, K: int, E: torch.Tensor) -> Tuple[torch.Tensor]:
    ...
    Returns:
    - rays_origin: Tensor of shape (W, H, 3) representing the origin of rays.
    - rays_direction: Tensor of shape (W, H, 3) representing the direction of rays.
    """
    y, x = torch.meshgrid(
    ...
    assert rays_direction.shape == rays_origin, "[!] The ray origin-direction pair must match"
...
def render(W: int, H: int, K: torch.Tensor, E: torch.Tensor) -> torch.Tensor:
    ...
    """
    rays_origin, rays_direction = generate_rays(W, H, K, E)
